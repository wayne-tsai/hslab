...
static BLEUUID serviceUUID("16001e21-1e24-47ff-a0e4-b41fbad50058");
// The characteristic of the remote service we are interested in.
static BLEUUID    charUUID("d609117c-7027-4e19-a8a9-737de429c210");
...
static void notifyCallback(
  ...
  bool isNotify) {
    ...
    if (length >= sizeof(buffer)) {
        length = sizeof(buffer) - 1;  // Prevent overflow
    }
    memcpy(buffer, pData, length);
    buffer[length] = '\0';  // Null-terminate the string

    int distance = 0;
    sscanf(buffer, "Distance: %d cm", &distance);
